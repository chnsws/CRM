...
<style>
	...
	#mod4 img{width:60%;margin-top:10px;display: none;}
		...
		<div id="mod5">
			...
			<table>
				...
				<tr>
					<td>
						爱客CRM app<br>
爱客CRM是专注移动化，以销售为导向，提高销售转化的新型CRM。<br>
	...
	<div id="right-box">
		<div id="mod4">
			<div style="height:40%;margin-bottom:0px;padding:0;" >
				<center><?php echo ($_COOKIE['gsimg']); ?></center>
			</div>
...
$(function(){
	...
	//公司图片处理
	$("#mod4").find("img").css("height",parseInt((parseInt($("#mod4").css("width"))*6)/10)+'px');
	$("#mod4").find("img").fadeIn();
	//快捷按钮1
	$("#fast_button1").on("click",function(){
		$(".layui-nav-tree",parent.document).find("li").eq(1).children("dl").children("dd").eq(0).click();
	});
	//快捷按钮2
	$("#fast_button2").on("click",function(){
		$(".layui-nav-tree",parent.document).find("li").eq(2).children("dl").children("dd").eq(0).click();
	});
	//快捷按钮3
	$("#fast_button3").on("click",function(){
		$(".layui-nav-tree",parent.document).find("li").eq(3).children("dl").children("dd").eq(0).click();
	});
