        ...
        <div id="info-body">
            <table cellSpacing="0" cellpadding="0"  >
                <tr>
                    ...
                    <td rowspan="5"><span><img id="headimg" src="<?php echo ($_GET['public_dir']); ?>/head-img/<?php echo ($gsxxarr['gsxx_img']); ?>"></span><input type="file" name="headimg" lay-type="images" class="layui-upload-file" id="imm"></td>
                </tr>
                <!--合并后的空单元格-->
                <tr>
                ...
                <tr>
                    ...
                    <td id="gs_name"><?php echo ($gsxxarr['gsxx_name']); ?></td>
                </tr>
                <tr>
                    <td class="listname">企业简称</td>
                    <td id="qy_jiancheng"><?php echo ($gsxxarr['gsxx_jc']); ?></td>
                </tr>
                <tr>
                    <td class="listname" value="222">所属行业</td>
                    <td id="ss_hangye"><?php echo ($hangye[$gsxxarr['gsxx_hy']]); ?><input type="hidden" id="hid_hy_val" value="3" /></td>
                </tr>
                <tr>
                    <td class="listname">所在地区</td>
                    <td id="sz_diqu"><?php echo ($gsxxarr['gsxx_dq']); ?><input type="hidden" id="hid_dq_val" value="3" /></td>
                </tr>
                <tr>
                    <td class="listname">公司地址</td>
                    <td id="gs_address"><?php echo ($gsxxarr['gsxx_dz']); ?></td>
                </tr>
                <tr>
                    <td class="listname">公司规模</td>
                    <td id="gs_guimo"><?php echo ($gsguimo[$gsxxarr['gsxx_gm']]); ?><input type="hidden" id="hid_gm_val" value="3" /></td>
                </tr>
            </table>
        </div>
    </div>
...
<script src="<?php echo ($_GET['root_dir']); ?>/Public/index_js_css/datas/area_data.js"></script>
<script>
    ...
    function changetext()
    {
       ...
       //将编辑按钮改为保存按钮
        ...
        if("<?php echo ($gsxxarr['gsxx_dq_bm']); ?>"=='')//如果数据库中没有存地区，则显示默认的北京地区
        {
            dqbmarr=['110000','110100','110001'];
        }
        else//如果数据库中有地区数据，就显示数据库中存的数据
        {
            var dqbm="<?php echo ($gsxxarr['gsxx_dq_bm']); ?>";
        ...
        $("#dqsel2").html(dqsel2);
        $("#dqsel3").html(dqsel3);
        $("#hangyesel").val("<?php echo ($gsxxarr['gsxx_hy']); ?>");
        $("#gsgmsel").val("<?php echo ($gsxxarr['gsxx_gm']); ?>");
        liandong();
    }
    //修改完之后的保存操作
    function baocun()
    {
        var oldgsmc="<?php echo ($gsxxarr['gsxx_name']); ?>";//修改之前的公司名称
        var oldgsjc="<?php echo ($gsxxarr['gsxx_jc']); ?>";//修改之前的公司简称
        var oldshhy="<?php echo ($gsxxarr['gsxx_hy']); ?>";//修改之前的所属行业
        var oldszdq="<?php echo ($gsxxarr['gsxx_dq_bm']); ?>";//修改之前的所在地区编码
        var oldgsdz="<?php echo ($gsxxarr['gsxx_dz']); ?>";//修改之前的公司地址
        var oldgsgm="<?php echo ($gsxxarr['gsxx_gm']); ?>";//修改之前的公司规模
