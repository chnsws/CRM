...
<!--end!!-->
		...
		tr{line-height:28px}
			...
			.addshangji_weizhi{margin:0px auto;margin-top:20px;margin-left:20%;}	

			#nchangyong{margin-top:10px;margin-left:15%;}
			.onError{color:red;}
			.onSuccess{color:green;}
			.kongzhiyeshu{margin-top:19px;height:40px;font-size:16px;margin-left:20px;}
			.panel-title{margin-left:10px;margin-top:9px; font-size:21px; color:#1AA094;}
						.searchable-select-dropdown{z-index:99999990;}
						th{background-color:#50BBB1;height:30px;color:white;} 
   			 </style>
	...
	<body style="position:relative">
		...
		<div>
			  <ul>
			    <li class="panel-heading"><span class="panel-title"><b>客户</b></span><span class="xinzeng2"><button  onclick="add_yh()" class=" layui-btn layui-btn-small">新增用户</button><span><button onclick="daoru()" style="margin-left:20px" class=" layui-btn layui-btn-small">导入客户</button></span></li>
   			...
   			<div class="kehu2" >   
		   		<div class="kehu">   
				   		<div class='kongzhiyeshu uk-form '><span class="ycyo">每页显示<select  onchange="kongzhiyeshu(this)">
				   						...
				   						<?php if($list_num != ''): ?><option value="<?php echo ($list_num); ?>" selected="selected"><?php echo ($list_num); ?></option>
				   						<?php else: endif; ?>
				   					</select>条
					...
					<div id="dialog-form" style="display: none;border:1px;width:689px"> <!--新增弹出框-->
						<form id="ht_form" style="border:1px;width:688px" method="post" class="uk-form " >
							<table class="addshangji_weizhi " style="border:1px;width:688px">
								<?php echo ($show_bt); ?>
								<?php echo ($show_bt1); ?>
								<?php echo ($show_bt2); ?>
								<?php echo ($jw); ?>
 	...
 	layui.use('layer', function(){
							...
							var win=layer.open({
								...
								btn1:function(){
									...
									$.get(rooturl+'/index.php/Home/Kehu/pl_zhuanyi',{"id":fuzeren,"ziduan":fuzeren1,"kh_id":piliang_str},
		        		 				function(html){
