...
ul{list-style:none;}
body,html,#box{height:auto;margin:0;background-color:#F7F7F7;}
/*div{border:1px solid #f00;}*/
#box{height:auto;}
#main_table thead th{background: #50BBB1;border:none;color:#fff;}
#main_table{margin-top:0px;}
...
#right-body{margin:0px 10px 0 10px;}
#right-body-top{background:#f7f7f7;padding-top:10px;padding-bottom:10px;}
...
#sxdiv{background-color:#f7f7f7;margin:10px 10px 0px 10px;padding:10px 5px 10px 5px;}
#sxshowbtn{background-color:#f7f7f7;width:40px;text-align:center;margin:0 auto;cursor:pointer;}
...
/*分页*/
#page_div{float:right;}

.layui-layer-btn0{background:#1AA094;}
</style>
</head>
<body>
<div id="box">
    ...
    <div id="right-div">
        ...
        <!--筛选结束-->
        <div id="right-body">
            <div id="right-body-top" class="uk-form"  data-uk-sticky >
                <div id="rb-left">
                    ...
                    <button class="layui-btn layui-btn-primary" onclick="to_cpfl()"><i class='fa fa-reply' aria-hidden='true'></i>返回</button>
            ...
            <span id="tabletishi" style="display:none;">提示：可以按住键盘Shift键并滑动鼠标滑轮来横向滑动列表</span>
            <div id="right-body-body" style="overflow-X:auto;height:77;width:100%;" >
                <table class="layui-table" id="main_table" style="display:none;min-width:100%;" >
                    <thead>
                        <?php echo ($thstr); ?>
                    </thead>
                    <tbody>
...
window.page_fl_id="<?php echo ($_GET['flid']); ?>";
window.page_nowpage="<?php echo ($_GET['page']); ?>";
window.page_maxpage=<?php echo ($page_max_page); ?>;
page_nowpage=page_nowpage?page_nowpage:1;
...
layui.use(['laypage', 'layer'], function(){
    ...
    layui.laypage({
        ...
        jump: function(obj, first){
            ...
            check_page=check_page?check_page:1;
            if(check_page==page_nowpage)
            {
                //防止第一次进入本页时触发
                return;
            }
            //获得分类id
            var fl_id="<?php echo ($_GET['flid']); ?>";
            window.jiazai=layer.load(2);
            window.location=root_dir+"/index.php/Home/Chanpin/index?flid="+fl_id+"&page="+check_page;
        }
    });
});
//筛选模块-------------------
...
function sxfun()
{
    ...
    $.post(root_dir+"/index.php/Home/Chanpin/shaixuan",{"ajaxstr":ajaxstr,"px":px,"pz":pz,"flid":page_fl_id},function(res){
...
function daochu()
{
    layui.use('layer', function(){
        ...
        layer.open({
            type:0,
            title: '导出',
            content:"是否导出该分类的产品数据？",
            yes:function(index,layero){
                window.location=root_dir+"/index.php/Home/Chanpin/daochu_data?flid="+page_fl_id;
                layer.close(index);
            }
        }); 
    });  
    ...
    layui.use('layer', function(){
        ...
        var daoruopen=layer.open({
            ...
            btn1:function(index,layero){
                window.location=root_dir+"/index.php/Home/Chanpin/get_muban?flid="+page_fl_id;
            },
            ...
            btn2:function(index,layero){
                ...
                $.get(root_dir+"/index.php/Home/Chanpin/daoru_chanpin",{"csvfilename":is_upload,"flid":page_fl_id},function(res){
                    //alert(res);
                    if(res=='1')
                    {
                        $.get(root_dir+"/index.php/Home/Chanpin/del_old_file",{"oldname":"linshi/"+is_upload});
...
function cp_add()
{
    ...
    $("#cpimg").hide();
    //添加产品时如果有分类id就默认选中该分类
    var main_flid="<?php echo ($_GET['flid']); ?>";
    if(main_flid)
    {
        $("#addflsel").attr("disabled",true);
        $("#addflsel").find("option[value='"+main_flid+"']").attr("selected",true);
    }
    else
    {
...
function searchfun()
{
   ...
   // window.location=root_dir+"/index.php/Home/Chanpin/index?page="+page_nowpage+"&flid="+page_fl_id+"&searchInfo="+sea_type+","+sea_text;
    var px=JSON.stringify(nowpagepx);
    var pz=JSON.stringify(nowpagepz);
    //alert(sea_text+sea_type);return false;
    //alert(page_fl_id);
    $.post(root_dir+"/index.php/Home/Chanpin/searchfun",{"sea_type":sea_type,"sea_text":sea_text,"px":px,"pz":pz,"flid":page_fl_id},function(res){
        $("#right-body-body").children("table").find("tbody").html(res);
        if(sea_text=='')
        {
            $("#page_div").show();
...
{
    window.location=root_dir+"/index.php/Home/Chanpin/chanpininfo?flid="+page_fl_id+"&cpid="+thisid;
}
//添加产品分类
    ...
    layui.use('layer', function(){
        ...
        var hisopen=layer.open({
            ...
            btn1:function(){
                ...
                $.post(root_dir+"/index.php/Home/Chanpin/show_option",{"checkid":checkid,"flid":page_fl_id},function(res){
                    if(res=='1')
                    {
                        location.reload();
                    }
...
function to_cpfl()
{
    window.location=root_dir+"/index.php/Home/Cpfl/cpfl_index";
}
//表格排序
function table_px()
{
