...
class ShenpiController extends Controller {
	...
	//	$sp_map['sp_jg']=0;
		...
		{
			$sp_sql=$sp_base->where($sp_map1)->select();
		}else{
	...
	//	var_dump($huikuan4);exit;
		...
		if(cookie('user_fid')=='0')
		{
			$sp_sql_kp=$sp_base->where($sp_map_kp1)->select();
		}else{
						...
						foreach($kp_sp_show as $k=>$v)
						{
								$kp_show.="<tr>
									...
									$kp_show.="</td>
										...
										{
											$kp_show.="	<td><span style='color:#07d;cursor:pointer' class='".$v['kp_id']."' onclick='fapiaoxinxi(this)'>".$kp_type_arr[$v['kp_type']]."</span></td>";
										}else{
					...
					//var_dump($kaipiao);exit;
						foreach($kp_sp_show1 as $k=>$v)
						{
								$kp_show1.="<tr>
										...
										if($v['kp_type']==0 || $v['kp_type']==1)
										{
											$kp_show1.="	<td><span style='color:#07d;cursor:pointer;' class='".$v['kp_id']."' onclick='fapiaoxinxi(this)'>".$kp_type_arr[$v['kp_type']]."</span></td>";
										}else{
